...
cabx_create()
{
    ...
    if (result && option && entries && source_path_entry_map) {
        result->ref_count = 1;
    ...
    char* const * argv)
{
    ...
    while (1) {
        int opt;
        opt = getopt_long(argc, argv, "i:o:t:c:h", options, NULL);
...
"-o, --output= [OUTPUT]     specify cab file path.\n"
"-h                         show this message\n",
        exe_name);
    ...
    CABX_OPTION* opt)
{
    if (opt) { 
        ...
        cabx_option_set_cabinet_name(opt, "data%d.cab");
        cabx_i_mem_free(opt);
    }
}
